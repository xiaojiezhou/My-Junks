...
%macro LousyPrint(data=_last_,
	...
	font=Arial, fontSize=11, footnoteFS=9,
	...
	frame=box, rules=all, borderWidth=1.5,
	idJustify=left,
	titleJustify=center,
	...
	tocBreak=3, tocLink=EndofTextTable, order=internal
);
			...
			compute before _page_ / style()=[nobreakspace=false just=&titleJustify];
				%do i = 1 %to &nTitles;
					...
					%if &i=1 and &preTitle^=%str() %then %do;
						text&i="&escapeChar.R%str(%')\ql\fs%eval(&footnoteFS*2) %replaceTags(str=%quote(&preTitle))\par\fs%eval(&fontSize*2)\q%substr(&titleJustify,1,1) %str(%')"||strip(text&i);
					%end;
...
%macro rtfTable(str);
	...
	%local data escapechar splitchar font fontSize footnoteFS cellindent cellpadding headercolor
		   ...
		   optionStr nTitle nFootnote titleJustify footnoteJustify headertext footertext;
	%let data=_last_;
	%let order=internal;
	%let escapechar=%str(\);
	%let splitchar=%str(|);
	...
	%let idJustify=left;
	%let tocBreak=3; %let tocLink=EndofTextTable;
	%let nTitles=0;
	%let nFootnotes=0;
