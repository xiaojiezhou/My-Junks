...
* Upcates 10/06/2013(Yasser): 
*							1-random effects are entered in CLASSVAR, in addition to optional_random_statement;
*                       	    e.g, CLASSVAR=TRTVAR COVAR SUBJECT and optional_random_statement=random subject;
*						    2- Interactions are entered in CovVar. Their levels are displayed in SolutionFout dataset;
*                              in EffectLev Column (e.g, TRT*Baseline will only have the levels of TRT displayed in EffectLev column; 
...
* Create a list of trtvar and categorical covariates to be included in the solutionf ouptut data from proc mixed;
...
*Updates 03/13/2013(Yasser): added by statement in merging the fixed effects solutions from analysis of full data and outliers-excluded data;
proc sort data=solutionF;
by &byvar. effect;* &ClassVar. ;
run;

proc sort data=R_solutionF;
by &byvar. effect;* &ClassVar. ;
run;
...
** Create dummies for levels of treatment and categorical covariates;
data &SolutionFOut;
...
select %do f=1 %to &nbyvars; &&byvar&f %end; ,effect,EffectLev,All_Estimate,Prob_t_ALL,R_Estimate,Prob_t_R
from &SolutionFOut
