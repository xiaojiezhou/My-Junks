...
%DO %WHILE (%eval(&COUNT)<=%eval(&COUNT1));
	...
	%do;
		proc sort data=&&D&COUNT. out=tempA_&COUNT.;
			by &Sortvar &classvar.a;
		run;
		%if (%eval(&COUNT2.)>%eval(&COUNT1.)) %then
		%do;
			proc sort data=&&D&Count2. out=tempB_&COUNT2.; 
				by &Sortvar &classvar;
			run;
				    
			data tempA_&COUNT.;
            	merge tempA_&COUNT.(in=i) tempB_&COUNT2.(in=j rename=(&classvar=&classvar.a)) ;
				by &Sortvar &classvar.a;
				if i=1;
		...
		proc report data=tempA_&COUNT. nowd headline split='?'  spacing=0 box style(header)=[background=white font_size=10 pt  ];
			column _name_ &svar1. attribute &svar1.=temp &svar  number &classvar.a grouping 
			lsmean %if (%eval(&COUNT2.)>%eval(&COUNT1.)) %then estimate; stderr pval;
			...
			compute Attribute/char length=100;
