...
* User Input1: Define the directory for the input data set;
%LET Ddata		=C:\Documents and Settings\ti4224\My Documents\My Work\Toolbox\Macros\Macro Templates\Current Version\SSS\Testing;

* User Input2: Define the directory for macros catalogue;
*%LET catl		= /data/Trainning/Sunita/P3-PG/P4; 
                   
* User Input32: Define the directory for the SAS codes;
%LET Dcodes		=C:\Documents and Settings\ti4224\My Documents\My Work\Toolbox\Macros\Macro Templates\Current Version\SSS\Testing;

* User Input4: Define complete path (including filename) for output file;                    
%LET Foutput	=C:\Documents and Settings\ti4224\My Documents\My Work\Toolbox\Macros\Macro Templates\Current Version\SSS\Testing;

libname in "&Ddata";

%LET Title		= CRB 10-04-051 SSS-15;		        * User Input5 : Define the title for the study;
%LET Test		= 0.1;     		                * User Input6 : Define the p-value for testing significance;
%LET Indata		= in.CRB1004051Biomarkers_2b;		* User Input7 : Define the input data set;
...
%Let ResultsFileName=CRB 10-04-051 SSS15 Biomarkers;           
...
proc mixed data=datause;* noitprint noclprint noinfo;	*Proc Mixed1;
   ...
   ods rtf exclude all;
   ods output lsmeans = LsmBase diffs = DiffsBase;
run;
...
proc mixed data=datause;* noitprint noclprint noinfo;		*Proc Mixed2;
  ...
  lsmeans trt / diffs cl alpha=.1;
  ods rtf exclude all;
  ods output lsmeans = LsmPostTrt diffs = DiffsPostTrt ;
run;
...
proc mixed data=datause;* noitprint noclprint noinfo;		*Proc Mixed3;
  ...
  lsmeans trt / diffs cl alpha=.1; 
  ods rtf exclude all;
  ods output lsmeans = in.&Var._LsmDiff diffs = DiffsDiffVar;
run; 
...
%APPEND(    condition1  =StudyDay=1;                    , 
			input1      =LsmBase DiffsBase           , 
			condition2  =StudyDay=21;                ,
			input2      =LsmPostTrt DiffsPostTrt     , 
			output      =in.&Var._LsmApp in.&Var._DiffsApp      );
...
*%REPORT_SSS_TRTDIFF(ESTIMATES=in.&Var._LsmDiff, DIFFERENCE=DiffsDiffVar);


*This macro creates Adjusted Mean Table;
*%REPORT_SSS_ABS(LibnameOutputDS=in, InputDSN=DataUse, APP_ESTIMATES=in.&Var._Lsm_app, APP_DIFFERENCE=in.&Var._Diffs_app, ESTIMATES=in.&Var._LsmDiff);

*This macro creates Adjusted Mean Table for Biomarkers;
%REPORT_SSS_ABS_GeoMean(LibnameOutputDS=in, InputDSN=DataUse, APP_ESTIMATES=in.&Var._LsmApp, APP_DIFFERENCE=in.&Var._DiffsApp, ESTIMATES=in.&Var._LsmDiff);
