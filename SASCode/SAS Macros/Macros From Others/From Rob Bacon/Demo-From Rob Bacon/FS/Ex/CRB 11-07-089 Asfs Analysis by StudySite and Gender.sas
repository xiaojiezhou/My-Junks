...
%LET Title		= Study CRB 11-07-089 BC-171 ASFS Results by Study Site and Gender;            	* Define the title for the study;
%LET Indata		= in.crb1107089_2b; 	* Define the input data set;
%LET Sortvar	= studyday studysite gender;       				* List all the by variables. In case of more than one by variable,list them space seperated e.g. SORTVAR = Visit Age Side;
...
%LET VarList	= AsfsTotal;               	* List all the dependent variable. In case of more than one variable, list them space separated;

%LET ResultsFileName=CRB 11-07-089 BC-171 ASFS Results by Study Site and Gender;
...
data datause;	
	...
	*outliers, instrument error etc. data points can be dropped out here;
	if studyday in (7,14) then delete;
run;
...
%DO %WHILE ( %SCAN(&varlist, %EVAL(&DEP_COUNT)) NE );;
	...
	*	ods select studentpanel;
	...
	proc mixed data = datause noitprint noclprint noinfo;	
		...
		by &Sortvar;
		class trt;
		model &var.= trt age Base&Var. /residual;
		*random RandNo;
		lsmeans trt / diffs cl;
		ods rtf exclude all;
	*	ods select studentpanel;
	...
	proc mixed data = datause noitprint noclprint noinfo;  
		...
		by &Sortvar;
		class trt ;
		model Diff&Var.= trt age Base&Var. /residual;
		*random RandNo;
		lsmeans trt / diffs cl;
		ods rtf exclude all;
	*	ods select studentpanel;
...
*	%ReportAcrossGrp
 ...
 /*
ods graphics / reset noborder width=600px height=900px
