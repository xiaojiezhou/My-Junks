...
%macro anal(endpt=);
ods output lsmeans=lsmeans_&endpt diffs=diffs_&endpt;
proc mixed data=t1;
class trt gender;
model &endpt=gender age trt;
lsmeans trt/pdiff;
by visit;
...
%mend;
%anal(endpt=LogHODE);
%anal(endpt=LogHODEC18ratio);
%anal(endpt=logSQ_OOH);
%anal(endpt=logSQ_OOHN);
%anal(endpt=logSQ_OxideN);
%anal(endpt=LogHistamineN);

Title "LogHODE";
%letter(lsmdata=lsmeans_LogHODE,diffdata=diffs_LogHODE,byvars=visit,alpha=0.05,outdata=outdata, trtvar=trt);
proc sort data=outdata; by visit trt ; proc print data=outdata; run; 

Title "LogHODEC18ratio";
%letter(lsmdata=lsmeans_LogHODEC18ratio,diffdata=diffs_LogHODEC18ratio,byvars=visit,alpha=0.05,outdata=outdata, trtvar=trt);
proc sort data=outdata; by visit trt ; proc print data=outdata; run;

Title "logSQ_OOH";
%letter(lsmdata=lsmeans_logSQ_OOH,diffdata=diffs_logSQ_OOH,byvars=visit,alpha=0.05,outdata=outdata, trtvar=trt);
proc sort data=outdata; by visit trt ; proc print data=outdata; run;

Title "LogSQ_OOHN";
%letter(lsmdata=lsmeans_LogSQ_OOHN,diffdata=diffs_LogSQ_OOHN,byvars=visit,alpha=0.05,outdata=outdata, trtvar=trt);
proc sort data=outdata; by visit trt ; proc print data=outdata; run;

Title "logSQ_OxideN";
%letter(lsmdata=lsmeans_logSQ_OxideN,diffdata=diffs_logSQ_OxideN,byvars=visit,alpha=0.05,outdata=outdata, trtvar=trt);
proc sort data=outdata; by visit trt ; proc print data=outdata; run;

Title "LogHistamineN";
%letter(lsmdata=lsmeans_LogHistamineN,diffdata=diffs_LogHistamineN,byvars=visit,alpha=0.05,outdata=outdata, trtvar=trt);
proc sort data=outdata; by visit trt ; proc print data=outdata; run;
...
run;
/********
%let lsmdata=lsmeans_logSQ_OOH;
%let diffdata=diffs_logSQ_OOH;

%let lsmdata=lsmeans_LogHODEC18ratio;
%let diffdata=diffs_LogHODEC18ratio;
%let byvars=visit;
%let alpha=0.05;
%let outdata=outdata;
%let trtvar=trt;
