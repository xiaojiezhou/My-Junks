...
set &printdata; 
  flg_probt=0; 
  if 0 < abs(probt)<= 0.05 then flg_probt=2;
  else if 0.05 < abs(probt)<= 0.10 then flg_probt=1;
  else flg_probt=0;
  flg_probtr=0; 
  if 0 < abs(probtr)<= 0.05 then flg_probtr=2;
  else if 0.05 < abs(probtr)<= 0.10 then flg_probtr=1;
  else flg_probtr=0;
...
%do i=1 %to &ncomps;
  ...
  flg_probr&i=0; 
  if 0 < abs(probr_&i)<= 0.05 then flg_probr&i=2;
  else if 0.05 < abs(probr_&i)<= 0.10 then flg_probr&i=1;
  else flg_probr&i=0;
  flg_wilcx&i=0;
...
column &byVar &TrtVar RespVar
		 ("Group Level Summary of &TrtVar"  n mean Stddev Stderr Median Min Max P25 P75
						lsmean lsmeanr se ser probt probtr)
         ("Pairwise Group Comparison of &TrtVar" 
		%do i=1 %to &ncomps; 
			%if &i<&ncomps %then ("&&comp&i" est_&i estr_&i se_&i ser_&i  prob_&i probr_&i wilcox_&i);
			%if &i=&ncomps %then ("&&comp&i" est_&i estr_&i se_&i ser_&i  prob_&i probr_&i wilcox_&i));
		%end;
;
column flg_probt flg_probtr %do i=1 %to &ncomps; flg_prob&i flg_probr&i flg_wilcx&i %end;;
column dummy;
%if %length(&byvar)>0 %then
 ...
 /** ALL DATA **/
   ...
   define probt / display "Pvalue*ALL DATA" order=data center format=pvalue6.4  style(column)=[width=150];
%do i=1 %to &ncomps;
...
/** EXTREME VALUES REMOVED **/
   ...
   define probtr  / display "Pvalue*Xclud Extrm" order=data center format=pvalue6.4  style(column)=[width=150];
%do i=1 %to &ncomps;
   ...
   define probr_&i 	/ display "Pvalue*Diff*Xclud Extrm" order=data center format=pvalue6.4  style(column)=[width=150];
%end;
   ...
   define dummy / computed noprint;
	compute dummy;
	   if flg_probt gt 0 then do;
        call define("probt",'style','style=[background=' ||put(flg_probt,FlagFmt.) || ']');
	   end;
	   if flg_probtr gt 0 then do;
        call define("probtr",'style','style=[background=' ||put(flg_probtr,FlagFmt.) || ']');
	   end;
	%do i=1 %to &ncomps;
	   ...
	   if flg_probr&i gt 0 then do;
        call define("probr_&i",'style','style=[background=' ||put(flg_probr&i,FlagFmt.) || ']');
	   end;
