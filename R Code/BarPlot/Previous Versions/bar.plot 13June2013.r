...
path<-"Q:\\beauty_care\\clinical\\Biometrics\\Lin Fei\\2012\\CRB 12-04-054 - Surf and Turf\\Xiaojie\\Plots"
...
#---- Washed Away:  by visit (BL, CFB, Endpt) ---#
subtitle1="Measured by OFDA "
bar.spc1 <-c(2,1,2,1,2,1,4,1,2,1,2,1,4,1,2,1,2,1)
bar.color1<-rep(c("deepskyblue4", "aliceblue", "darkolivegreen3", "deepskyblue4", "darksalmon", "deepskyblue4"),3)
length(bar.color1)

plotdata=indata[ (indata$group2.names =="Washedaway") ,]
plotdata


bar.plot (trts=plotdata$trts, 
          group1.names= unique(plotdata$group1.names), 
          group1.hpos=c(4,10, 15), 
          group1.side=3,
          group1.line=1,
          group2.names= c(" "), 
          group2.hpos=c(2),
          ...
          group2.line=1,
          ymin=-2, ymax=6,
          trtcmp.pvalue=round(as.numeric(plotdata$trtcmp.pvalue), digits = 2),
          trtcmp.pvalue.xadj=1,
          arrow="-------------------------- ", 
          ylable.right=" ",
          ylabel.left="Unwashed - washed diameter Difference (um)", 
          main.title="Hair Diameter Washed Away")
...
bar.plot <- function(trts=plotdata$trts, 
  ...
  bp<-barplot2(as.numeric(bar.height), col=bar.color, 
               space=bar.spc,xpd=F,plot.ci = TRUE,ci.l = bar.height - bar.se, ci.u = bar.height + bar.se,
  ...
  for (pv in 1:length(trtcmp.pvalue)){
    ...
    if(!is.na(trtcmp.pvalue[pv])){
      if (trtcmp.pvalue[pv]!="."){
        mtext(side = 3, at=bp[pv]+trtcmp.pvalue.xadj, text=paste("p =",trtcmp.pvalue[pv]), cex=1.3, font=4, line=-2, col="red")
      }
    }
  }
